...
class MainService : LifecycleService() {
    ...
    private fun imageProxyToJpegOptimized(imageProxy: ImageProxy): ByteArray? {
        ...
        if (yuvBuffer == null || yuvBuffer?.size != size) {
            yuvBuffer = ByteArray(size)
        }
    ...
    private fun connectUsb() {
        ...
        if (driver == null) { updateUsbStatus("USB: No device found"); return }
    ...
    private fun openSerial(device: UsbDevice) {
        synchronized(usbLock) {
            ...
            if (port == null) {
                updateUsbStatus("USB: Open failed")
                return
            }
            serial = port
        }
        ...
        synchronized(usbLock) {
            ...
            status?.let { newStatus ->
                ...
                if (newStatus.contains("failed")) {
                    ...
                    val driver = UsbSerialProber.getDefaultProber().findAllDrivers(manager).firstOrNull()
                    if (driver == null) {
                        finalStatus = "USB: Disconnected"
                    }
                }
                
                updateUsbStatus(finalStatus)
                if (finalStatus.contains("failed")) scheduleUsbReconnect()
    ...
    private fun scheduleUsbReconnect() {
        if (isShuttingDown) return
        usbReconnectHandler.removeCallbacksAndMessages(null)
        usbReconnectHandler.postDelayed({
            if (serial == null) connectUsb()
        }, 2000L)
    }
    ...
    private fun startAckReader(port: UsbCdcSerial) {
        usbReadExecutor.execute {
            ...
            while (serial === port) {
                ...
                if (count > 0) {
                    if (bytesRead + count > buffer.size) {
                        bytesRead = 0
                    }
                    ...
                    if (i > 0) {
                        System.arraycopy(buffer, i, buffer, 0, bytesRead - i)
                        bytesRead -= i
                    }
                }
            }
