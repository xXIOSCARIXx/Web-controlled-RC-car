...
A real-time RC vehicle surveillance and control system. An Android phone mounted on an RC car streams live video, audio, GPS, and telemetry to a browser-based dashboard, while a browser gamepad drives the vehicle over the internet via a USB-connected Arduino.

```
[Browser + Gamepad] ──WSS──► [Node.js Relay Server] ──WSS──► [Android App]
                                                                     │
                                                               USB Serial
                                                                     │
                                                             [Arduino + Servos]
...
- The publisher WebSocket uses a self-signed TLS certificate to prevent rogue publishers from injecting video or commands on a shared network. The certificate is pinned in the Android app.
- The viewer port (3000) uses plain HTTP/WS. If you expose this to the internet, put it behind a reverse proxy (e.g. nginx with Let's Encrypt) and add authentication.
- No authentication exists between viewers and the server by default. Anyone who can reach port 3000 can watch the stream and send gamepad commands.
